...
Right now, community decisions are made through a democratic consensus process, which has worked well and allowed us to move forward effectively. Soon, however, we may move to formalize and alter this process in to set Electron up for the highest chance of long-term success and which i'll talk about in greater detail a bit later on during this talk.
...
Finally, how can you get involved in Electron? This is something we've worked to clarify over the past year by adding documentation around how our team approaches development and how you can find your niche within that. We've also been focusing on labeling issues as "good first issues" when we think it will help a contributor  familiarize themselves with the codebase. Core team members have also paired with developers looking to break into open source development. These efforts have prompted my team and me to improve our own workflows, as sometimes it's hard to see what's unclear or could be improved about things you look at day in and day out.
...
As in any distributed open source project, meetings are hard, but we work to accommodate everyone's needs as we maintain what is functionally a 24-hour development cycle. We also work as a cross-company team: even though we’re employed by different companies we meet with each other directly and attend all the same standups for Electron-related topics.
...
Regarding Node maintenance, we're working more closely with Node Core and its surrounding community, which allows us to anticipate changes between Node versions and prepare for how they'll affect our own code. We also maintain 20 or so patches we apply to each successive Node upgrade in our personal fork. While the patches work for now, we'd like to have to patch less so that our maintenance burden is correspondingly decreased. To that end, we're also now working with Node developers to upstream patches to Node core in a way that benefits everyone.

**Chromium:**

When I started, we were consistently 8ish versions behind Chromium's current stable version. Our goal has been to catch up ….. and while we haven't reached our goal yet, we've gotten progressively closer as time has passed. Right now Chromium, which releases every six weeks, is at version 67. Electron is using Chromium 66 in a beta cycle.
